...
export default function UserRegisterModule() {
    const initialFormData = {
      ...
      const handleSubmit = (e) => {
        ...
        if (IsValidate()) {
          ...
          axios.post('https://localhost:7194/api/User', formData)
  .then((response) => {
    console.log('User registered successfully:', response.data);
    displaySnackbar('User registered successfully', 'success');
  ...
  .catch((error) => {
    ...
    displaySnackbar('Error registering user', 'error');
  });
        }
      };
