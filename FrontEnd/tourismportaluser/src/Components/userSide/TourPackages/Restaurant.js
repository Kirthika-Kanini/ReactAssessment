    ...
    try {
      ...
      if (response.ok) {
       ...
       //   setErrorMessage('Error: ' + errorMessage);
        ...
        const matchingRestaurants = data.filter((restaurant) =>
          selectedPlace.restaurants.some((placeRestaurant) => placeRestaurant.restaurantId === restaurant.restaurantId)
        );
    ...
    {errorMessage}
      ...
      <aside className={`padding-5 ${isMobileMenuOpen ? 'show' : ''}`}>
        <div className="aside__items">
          ...
          <a href="#home" className="aside--active">
            Home
          </a>
          <a href="/home" className="nav--active">Home</a>
          <a href="/tour">Tour</a>
      ...
      <section className="destination padding-5" id="pricing">
        ...
        <div className="destination-box">
          {filteredRestaurants.map((restaurant) => (
            <div className="destination__item" key={restaurant.restaurantId}>
              <Link to={`/hotel/${restaurant.restaurantId}`} style={{ textDecoration: 'none', color: 'inherit' }}>
                <div className="destination-image">
                  <img src={`https://localhost:7036/uploads/restaurant/${restaurant.restaurantImage}`} alt="Hotel" />
                  <div className="price">{restaurant.restaurantSubLocation}</div>
                </div>
                <h2>{restaurant.restaurantName}</h2>
                <p className="description">{restaurant.restaurantDescription}</p>
